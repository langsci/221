...
In this book, I have investigated \isi{gemination} with the five English affixes \prefix{un}, negative \prefix{in}, locative \prefix{in}, \prefix{dis} and \suffix{ly}. I conducted a corpus and an experimental study. 
\tabref{tbl:Overview gemination in corpus and experimental study} summarizes the main results of both studies by displaying the \isi{gemination} pattern of each affix. Note that the \isi{gemination} pattern of locative and negative \prefix{in} is not displayed separately as there were no differences in \isi{gemination} between the two affixes. 
...
The comparison of the affix's \isi{segmentability} in both studies indicates that the two theoretically derived \isi{segmentability} hierarchies are borne out by the data.
 The corpus study showed the same \isi{segmentability} pattern across all \isi{decomposability measures}.
...
The experimental study, furthermore, showed effects of \isi{word-specific decomposability}. The two \isi{decomposability measures} \textsc{SemanticTransparencyRating} and log\textsc{RelativeFrequency} affected consonant duration with \suffix{ly}. Items which were rated as less \isi{decomposable} featured a shorter /l/ than items which were rated as more \isi{decomposable}. Furthermore, for \suffix{ly}-words with a \isi{syllabic} double consonant, derivatives with a higher \isi{relative frequency}, i.e. less \isi{decomposable} items, featured a shorter /l/ than derivatives with a lower \isi{relative frequency}, i.e. more \isi{decomposable} derivatives. However, as discussed in \sectref{ly experiment summary}, the effects of \isi{word-specific decomposability} are very small and the effect of \isi{relative frequency} might be caused by a few items in the data set. 


To sum up, the present study shows that categorical \isi{segmentability} and \isi{word-specific decomposability} may affect the \isi{acoustic duration} of complex words.
...
Effects of \isi{word-specific decomposability} were only found for the suffix \suffix{ly}, not for the prefixes. This suggests that suffixes might be more affected by \isi{word-specific decomposability} than prefixes.
 Crucially, the effects of \isi{word-specific decomposability} do not interact with \isi{gemination}. 
...
Gemination is a categorical phenomenon. This is evidenced by the bimodal distribution of singletons and doubles found in the data. Furthermore, \isi{gemination} is mostly governed by the affix, i.e. by a categorical factor. While some affixes \isi{geminate}, others do not. In addition to the affix itself, for some affixes, the \isi{stress} pattern of a derivative affects \isi{gemination}. 
As shown in the experimental study, \isi{gemination} does not depend on \isi{orthography}.

Even though \isi{gemination} is categorical in the sense that doubles are categorically longer than singletons, there are gradient differences in the \isi{degree of gemination} between affixes. The \isi{degree of gemination}, or the strength of \isi{gemination}, is mainly indicated by the durational differences between phonological doubles and phonological singletons. Stronger \isi{gemination} goes together with larger singleton-\isi{geminate} ratios. 


The prefix \prefix{un} clearly \isi{geminates} in both studies. In the corpus and the experimental study, phonological doubles (e.g. /nn/ in \textit{unnatural}) are clearly longer than singletons in complex words (e.g. /n/ in \textit{uneven} or \textit{untold}). The experimental study furthermore showed that doubles are longer than singletons in base words  (e.g. /n/ in \textit{natural}). Singleton-\isi{geminate} ratios are bigger in the experimental than in the corpus study.

The prefix \prefix{in} also \isi{geminates} but \isi{gemination} is weaker than \isi{gemination} with \prefix{un}. While \isi{gemination} with \prefix{in} is comparable to \isi{gemination} with \prefix{un} in the corpus study, in the experimental study \isi{gemination} with \prefix{in} is weaker and depends on \isi{stress}. 
 In the corpus study, 
...
the singleton-\isi{geminate} ratios for \prefix{in} are smaller than the ones for \prefix{un}.
Furthermore, doubles are only longer than singletons when the base-initial syllable of a derivative is \isi{stressed}, and doubles are only longer than some types of singletons. For /ɪn/, doubles (e.g. /nn/ in \textit{innumerous}) are longer than singletons in complex words followed by a vowel (e.g. /n/ in \textit{inefficient}), but not longer than singletons followed by a consonant (e.g. /n/ in \textit{intolerant}). For /ɪm/, doubles (e.g. /mm/ in \textit{immortal}) are longer than singletons in complex words followed by a consonant (e.g. /m/ in \textit{impossible}). Doubles in derived words are never longer than initial singletons in base words (e.g. /n/ in \textit{numerous} or /m/ in \textit{mortal}). 




The prefix \prefix{dis} \isi{geminates} in the corpus study as well as in the experimental study. In the corpus study, \isi{gemination} with \prefix{dis} is weaker than \isi{gemination} with \prefix{un} and \prefix{in} in the sense that there is a smaller singleton-\isi{geminate} ratio for \prefix{dis} than for \prefix{un} and \prefix{in}. Furthermore, one \prefix{dis}prefixed type did not \isi{geminate} (\textit{dissolution}), presumably because of its unstressed base-initial syllable. 

In the experimental study, \isi{gemination} with \prefix{dis} is weaker than \isi{gemination} with \prefix{un} but similar to \isi{gemination} with \prefix{in}.
All \prefix{dis}prefixed words in the experimental data geminated. However, no morphological \isi{geminates} with an unstressed base-initial syllable were included. 
The singleton-\isi{geminate} ratio for \prefix{dis} is smaller than that for \prefix{un}, and similar to that for \prefix{in}. 
Furthermore, in contrast to \prefix{un}, and similar to \prefix{in}, doubles are not longer than singletons in base words.  


For the suffix \suffix{ly}, no \isi{gemination} was found. In the corpus study, double consonants with \suffix{ly} were as long as singletons. In the experimental study, three different types of double consonants were investigated: \isi{syllabic} ones (e.g. /ll/ in  \textit{ment(a)lly}), non-\isi{syllabic} ones spelled with the orthographic sequence <lel> (e.g. /ll/ in \textit{solely}), and non-\isi{syllabic} ones spelled with <ll> (e.g. /ll/ in \textit{really}). While under certain conditions the \isi{syllabic} doubles and the doubles spelled with <lel> were longer than some types of singletons, there was no systematic difference between doubles and singletons, i.e. two underlying consonants are not longer than one. The suffix \suffix{ly} \isi{degeminates}.



Thus, the overall pattern of \isi{gemination} is the following: the prefix \prefix{un} \isi{geminates}. The prefixes \prefix{in} and  \prefix{dis} also \isi{geminate} but \isi{gemination} is weaker. For \prefix{in}, \isi{gemination} is only weaker than \isi{gemination} with \prefix{un} under experimental conditions. For \prefix{dis}, \isi{gemination} is weaker in both the corpus and the experimental study. The suffix \suffix{ly} \isi{degeminates}. The overall \isi{gemination} pattern of the affixes is shown in (\ref{gemination hierarchy}) in form of a hierarchy.
...
Apart from the studies presented in this book, three studies looked at affixational \isi{gemination} in English, \cite{Kaye.2005}, \cite{Oh.2012} and \cite{Kotzor.2016} (see chapter \ref{previous empirical work} for a detailed discussion). 
\cite{Kaye.2005} and \cite{Oh.2012} looked at \isi{gemination} with \prefix{un} and \prefix{in}. Their results that both prefixes \isi{geminate} fit in well with the results presented in this book.

\cite{Oh.2012} furthermore showed that the singleton-\isi{geminate} ratio is smaller for \prefix{in} than for \prefix{un}, i.e. \prefix{in} \isi{geminates} to a lesser degree than \prefix{un}, and that the difference in singleton-\isi{geminate} ratios between \prefix{un} and \prefix{in} is even larger in \isi{careful speech} than in \isi{normal speech}. 
This fits in well with this study's result that there are only differences in the \isi{degree of gemination} between \prefix{un} and \prefix{in} in the experimental data, which presumably is more similar to \isi{careful speech} than the corpus data.

\cite{Kotzor.2016} looked at \isi{gemination} with \suffix{ness} and \suffix{ly}. They claim that both suffixes \isi{geminate}. Their result that \suffix{ly} \isi{geminates} is not in line with the results presented in this book. However, as thoroughly discussed in chapter \ref{previous empirical work}, \cite{Kotzor.2016} do not provide separate analyses for the two affixes \suffix{ness} and \suffix{ly}. It is thus questionable how valid their result is. 
...
While for some affixes, there is a difference in the \isi{degree of gemination} between the corpus and the experimental study, for others no such difference was observed. For the prefix \prefix{un}, \isi{gemination} is stronger in the experimental study than in the corpus study. For \prefix{in}, the opposite is the case: \isi{gemination} is weaker in the experimental study than in the corpus study. For \prefix{dis} and \suffix{ly}, no difference was found in the degree of (de)\isi{gemination} between the corpus study and the experimental study.
...
One major difference between the data sets is their size. The experimental data set features more than 13 times more observations than the corpus study. In a \isi{linear model}, this would lead to serious problems, as the model's estimates would be largely based on the experimental data.
...
Returning to the observed differences, one might speculate that the different behavior of the affixes with regard to their \isi{gemination} pattern in the corpus and the experimental study is related to their \isi{segmentability}. It might, for example, be that highly segmentable affixes like \prefix{un} display stronger \isi{gemination} under experimental conditions, while less segmentable affixes like \prefix{in} show weaker \isi{gemination} under experimental conditions. However, this idea does not carry through. As described above, the prefix \prefix{dis} is approximately as segmentable as negative \prefix{in}, and presumably more segmentable than locative \prefix{in}. If the differences between corpus and experimental study were related to \isi{segmentability}, \prefix{dis} should behave as both \prefix{in}prefixes. It does not. %However, \prefix{dis} behaves differently than \prefix{in}. 
...
In addition to the affix, the variable \textsc{BaseInitialStress}, which is not predicted to govern \isi{gemination},  affected \isi{gemination} with the prefix \prefix{in} in the experimental study. 
...
According to \isi{Lexical Phonology}, the \isi{stratum} of the affix is decisive for \isi{gemination}. The \isi{level 1} affixes \prefix{in} and \prefix{dis} are predicted to \isi{degeminate}, and the \isi{level 2} affixes \prefix{un} and \suffix{ly} are predicted to \isi{geminate}. Except for the fact that the \isi{level 2} affix \prefix{un} \isi{geminates}, all other predictions are wrong. The \isi{level 1} affixes \prefix{in} and \prefix{dis} \isi{geminate}, and the \isi{level 2} affix \suffix{ly} \isi{degeminates}. Thus, the stratal prediction is clearly falsified by the data.

% Stratal OT
The predictions made by Stratal OT are very similar to the predictions made by \isi{Lexical Phonology} as they are also based on lexical \isi{strata}. 
The \isi{level 2} affixes \prefix{un} and \suffix{ly} are predicted to \isi{geminate}. The \isi{gemination} of the \isi{dual-level affixes} \prefix{in} and \prefix{dis} is predicted to depend on the type of base found in a pertinent word. Items with a bound root are predicted to \isi{degeminate}, and items with words as bases are predicted to \isi{geminate}. The \isi{degemination} of \suffix{ly}, and the fact that \isi{gemination} of \prefix{in} and \prefix{dis} is independent from a derivative's type of base, falsify the predictions made by Stratal OT.  

%Prosodic Word
The Prosodic Word Approach predicts that all affixes forming independent \isi{prosodic words} \isi{geminate}. All affixes not forming independent \isi{prosodic words} are predicted to \isi{degeminate}. According to the approach, \prefix{un} always forms an independent \isi{prosodic word} and is thus predicted to always \isi{geminate}, \suffix{ly} never forms an independent \isi{prosodic word} and is thus predicted to never \isi{geminate}. Gemination with \prefix{in} and \prefix{dis} is predicted to depend on the derivative the prefix is found in, i.e. on the \isi{prosodic word} status of the prefix in the  derivative. 
...
The Prosodic Word Approach  makes the correct predictions for \prefix{un} and \suffix{ly}. It also correctly predicts that \isi{gemination} with \prefix{in} and \prefix{dis} is not as consistent as \isi{gemination} with \prefix{un}. However, the approach fails to predict the correct \isi{gemination} pattern for \prefix{in} and \prefix{dis}. 
 Neither the \isi{semantic transparency} of a derivative nor its type of base significantly influences \isi{gemination} with \prefix{in} and \prefix{dis}.
...
Gemination with \prefix{in} in the experimental study depends on \isi{stress}. Only derivatives with a \isi{stressed} base-initial syllable \isi{geminate}. Furthermore, \isi{gemination} with \prefix{dis} might also depend on \isi{stress}. As discussed in section  \ref{discussion experiment}, the \isi{degemination} of the word \textit{dissolution} is probably caused by its unstressed base-initial syllable, i.e. there is some evidence that \prefix{dis} only \isi{geminates} when the base-initial syllable of a derivative is \isi{stressed}. 

Based on these results, one can speculate that using different criteria to determine \isi{prosodic word} status, such as \isi{stress}, might have led to better predictions of the Prosodic Word Approach for \prefix{in} and \prefix{dis}. 
However, using \isi{stress} to determine \isi{prosodic word} status is very problematic. % and was therefore desisted from in this study.
 According to \cite{Raffelsiefen.1999}, \isi{prefixal stress} is the crucial determiner for \isi{prosodic word} status, i.e. not base-initial \isi{stress} (see \sectref{prosodic word}). Prefixal \isi{stress} is, however, very difficult to determine and highly debated in the literature (see discussion on \isi{prefixal stress} in sections \ref{affixes} and \ref{stress coding}). Therefore, it was not directly investigated in this study. Instead the effect of base-initial \isi{stress} on \isi{gemination} was tested, but the exact relation of base-initial \isi{stress} and \isi{prefixal stress} is yet unclear.
Further studies which investigate the relation of base-initial \isi{stress}, \isi{prefixal stress} and \isi{prosodic word} status are needed to clarify the matter. Only if valid criteria for \isi{prosodic word} status are available, one can further investigate whether \isi{gemination} is governed by \isi{prosodic word} status. 
...
Gemination does not pattern according to the Non-Semantic Segmentability Hierarchy. According to that hierarchy, \suffix{ly} is more segmentable than \prefix{in} and \prefix{dis}. As \prefix{in} and \prefix{dis} \isi{geminate}, one should also find \isi{gemination} with \suffix{ly}. This is not the case. The suffix \suffix{ly} does not \isi{geminate}.  In turn, there is no support for affix-specific psycholinguistic approaches which predict \isi{gemination} to pattern according to the Non-Semantic Segmentability Hierarchy.


According to the Semantic Segmentability Hierarchy, \suffix{ly} is the least segment-able and the least informative affix. It is thus most likely to \isi{degeminate}. This assumption is supported by the data, i.e. we find \isi{degemination} with \suffix{ly}. There is also a difference in \isi{segmentability} and \isi{informativeness} between the four prefixes: \prefix{un} should \isi{geminate} to a higher degree than negative \prefix{in} and \prefix{dis}, which in turn should \isi{geminate} to a higher degree than locative \prefix{in}. As shown in (\ref{gemination hierarchy}), this pattern is at least partly observed, \prefix{un} \isi{geminates} to a higher degree than both \prefix{in}prefixes and \prefix{dis}. There is, however, no difference in the \isi{degree of gemination} between locative \prefix{in} and negative \prefix{in}, and \isi{gemination} with \prefix{dis} is a little weaker than \isi{gemination} with \prefix{in}. 


Overall though, the \isi{gemination} pattern supports the affix-specific psycholinguistic approaches which predict \isi{gemination} to pattern according to the Semantic Segmentability Hierarchy. The most segmentable and most informative affix \prefix{un} \isi{geminates}, the least segmentable and informative affix \suffix{ly} \isi{degeminates}, and the other affixes pattern in between.
...
Turning away from \isi{gemination}, the studies also revealed that \isi{decomposability} affects the \isi{acoustic realization} of complex words.
 In the corpus data, prefixal consonant durations for \prefix{un} and \prefix{in} reflects the \isi{segmentability} of the affix. 
The experimental study revealed gradient effects of \isi{decomposability} for \suffix{ly}-suffixed words. 
Both findings go in the same direction: the more \isi{decomposable} a unit is, the less \isi{reduction} is found. 
...
, there are still questions which remain unanswered. The studies found effects of categorical affix-specific \isi{segmentability}, as well as \isi{word-specific decomposability} effects. It remains unclear how these two different types of \isi{decomposability} effects interact. Furthermore, while there are effects of \isi{word-specific decomposability} for \suffix{ly}, there are no effects for the prefixes. This evokes the question of whether there is a difference between the retrieval and the processing of suffixed words on the one hand, and prefixed words on the other. Further studies are needed to investigate this difference and shed light on the interplay between categorical and word-specific effects of \isi{decomposability}. Only then can adequate \isi{models of word storage and retrieval} be specified. 

...
They were then asked to rate on a scale from 1 to 4 how easy it is to \isi{decompose} the words into two meaningful units. The participants were explicitly told which units to look for, e.g.\textit{ in} and the rest of the word for the rating of \prefix{in}prefixed words, and \textit{un} and the rest of the word for \prefix{un}prefixed words. The participants were given the opportunity to  indicate if they did not know a word. 
...
Thus, as in the \prefix{un}data set, in the \prefix{in}data set doubles are significantly longer than singletons.
...
\textsc{BaseInitialStress} affects consonant duration in the \prefix{in}model and in the \prefix{un} and \prefix{in} model. Before unstressed base-initial syllables the consonant is shorter than before \isi{stressed} base-initial syllables. 

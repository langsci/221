...
Note that only the experimental version of the \isi{decomposability rating} for the affixes \prefix{un}, \prefix{in} and \suffix{ly} is printed here. There were only minor alternations in the other versions of the \isi{decomposability rating}.\\
...
For example, the word \textit{uncool} can be broken down into two units: \textit{un-} and \textit{cool}. The unit \textit{\isi{un-} }is a unit which occurs at the beginning of many English words. In the word \textit{uncool} it has been added to the word \textit{cool} to make a new word which has the meaning of ‘not cool’. Thus, \textit{uncool} can be separated into two meaningful units.

The unit \textit{dis-} is another unit which is often attached to words to form new English words. One example is the word \textit{disconnect}. It can be broken down into the two meaningful units \textit{\isi{dis-} }(which in this case has a negative or reversing meaning) and \textit{connect}.

Another example of such a unit is \textit{-ly}, for example in the word \textit{possibly} which consists of the parts \textit{possible} and \textit{-ly}.  The unit\textit{ \isi{-ly} }has the meaning "in a certain manner" in English.
	...
	\label{tbl: corpus summary varables ly}
		...
		\resizebox{\textwidth}{!}{%
			...
			\begin{tabular}{lrrrrr}
				...
				\textbf{Categorical Predictors }& Levels &   &  &   & N\\ 
				\midrule
				\textsc{Environment}       & \texttt{l\#l} : 33 & \texttt{\isi{syllabic} l\#l}: 48 & \texttt{\#l}: 73 & & 154\\ 
				...
				& 35& 119  & & & \\ 					
				\textsc{PrecedingSegment}        &\texttt{ consonant}: 109 & \texttt{ vowel}: 145 & & &154 \\ 				\lspbottomrule 
			\end{tabular}
		}
...
\label{interactions corpus absolute durations}
		...
		\resizebox{\textwidth}{!}{%
			...
			\begin{tabular}{lrrrrr}
				\lsptoprule
		& \isi{un-}&\isi{in-}&\isi{un-} \& \isi{in-} & \isi{dis-} &\isi{-ly}\\
		\midrule
...
\label{interactions experiments complex models}
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{llrrrrrr}
				\lsptoprule
	 & &&&\textbf{\isi{un-}  }&  &\\
	\textbf{Two-way interactions} & \textbf{un-}&\textbf{in-}&\textbf{im-}&\textbf{\& \isi{in-} }& \textbf{\isi{dis-} }&\textbf{-ly}\\
	\midrule
...
\label{interactions experiments complete models}
	...
	\textbf{Two-way interactions}& \textbf{un-}&\textbf{in-}&\textbf{im-}& \textbf{\isi{dis-} }&\textbf{-ly}\\
	\midrule
...
\label{model in complex experiment}
	...
	\vspace*{-0.3cm}
		...
		\textsc{Environment}-\texttt{n\#C }&\color{lsLightGray} 0.001 & \color{lsLightGray}0.003 & \color{lsLightGray}60.047 & \color{lsLightGray}0.357 & \color{lsLightGray}0.722 \\ 
		...
		\textsc{Environment}-\texttt{n\#C:}&&&&&\\
		\textsc{Accentuation}-\texttt{unaccented }&\color{lsLightGray} 0.004 &\color{lsLightGray} 0.002 &\color{lsLightGray} 1054.188 &\color{lsLightGray} 1.428 &\color{lsLightGray} 0.154 \\ 
		\textsc{Environment}-\texttt{n\#V}:&&&&&\\
...
\label{model in complete experiment}
	...
	\vspace*{-0.3cm}
		...
		\textsc{Environment}-\texttt{n\#C }&\color{lsLightGray}  -0.001 & \color{lsLightGray} 0.001 & \color{lsLightGray} 119.097 &\color{lsLightGray}  -0.769 &\color{lsLightGray}  0.443 \\ 
		...
		\textsc{PrePause}-\texttt{pause} &\color{lsLightGray}  -0.001 &\color{lsLightGray}  0.001 &\color{lsLightGray}  1135.839 &\color{lsLightGray}  -0.985 &\color{lsLightGray}  0.325 \\ 
...
\vspace*{-0.3cm}
		...
		\textsc{Accentuation}-\texttt{unaccented }& \color{lsLightGray}0.002 &\color{lsLightGray} 0.002 &\color{lsLightGray} 1038.350 & \color{lsLightGray}1.408 & \color{lsLightGray}0.160 \\ 
...
\begin{table}
	...
	\label{model im complete experiment}
		...
		\textsc{PrePause}-\texttt{pause} & \color{lsLightGray}0.005 & \color{lsLightGray}0.004 &\color{lsLightGray} 1533.247 & \color{lsLightGray}1.299 & \color{lsLightGray}0.194 \\ 
		...
		\textsc{Environment}-\texttt{m\#C}:&&&&&\\
		\textsc{PrePause}-\texttt{pause} &\color{lsLightGray} -0.007 &\color{lsLightGray} 0.004 & \color{lsLightGray}1530.068 & \color{lsLightGray}-1.604 &\color{lsLightGray} 0.109 \\ 
		\textsc{BaseInitialStress}-\texttt{unstr.}:&&&&&\\
...
\label{model in un experiment}
		...
		\textsc{Environment}-\texttt{n\#C }&\color{lsLightGray} 0.012 & \color{lsLightGray}0.007 & \color{lsLightGray}592.421 & \color{lsLightGray}1.774 & \color{lsLightGray}0.077 \\ 
		
		\textsc{Environment}-\texttt{n\#V} & -0.015 & 0.007 & 524.169 & -1.996 & 0.046 \\ 
		
		\textsc{Affix}-\texttt{inNeg} &\color{lsLightGray} 0.012 &\color{lsLightGray} 0.007 &\color{lsLightGray} 508.387 &\color{lsLightGray} 1.636 & \color{lsLightGray}0.103 \\ 
		...
		\textsc{Environment}-\texttt{n\#C}:&&&&&&\\
		\textsc{Affix}-\texttt{inNeg} &\color{lsLightGray} -0.012 &\color{lsLightGray} 0.008 &\color{lsLightGray} 462.937 & \color{lsLightGray}-1.621 &\color{lsLightGray} 0.106 \\ 
		
		\textsc{Environment}-\texttt{n\#V}:&&&&&&\\
		\textsc{Affix}-\texttt{inNeg} &\color{lsLightGray} -0.016 & \color{lsLightGray}0.008 & \color{lsLightGray}438.960 & \color{lsLightGray}-1.946 & \color{lsLightGray}0.052 \\ 
		...
		\textsc{PrePause}-\texttt{pause} & \color{lsLightGray}0.011 & \color{lsLightGray}0.007 & \color{lsLightGray}2991.839 &\color{lsLightGray} 1.713 &\color{lsLightGray} 0.087 \\ 
		...
		\textsc{PrePause}-\texttt{pause }&\color{lsLightGray} -0.010 &\color{lsLightGray} 0.007 & \color{lsLightGray}2996.499 & \color{lsLightGray}-1.509 & \color{lsLightGray}0.131 \\ 
...
\label{model dis complex experiment}
		...
		\textsc{Accentuation}-\texttt{unaccented} & \color{lsLightGray}0.006 & \color{lsLightGray}0.004 & \color{lsLightGray}748.006 & \color{lsLightGray}1.514 & \color{lsLightGray}0.131 \\ 
...
\label{model dis complete experiment}
		...
		\textsc{Accentuation}-\texttt{unaccented} & \color{lsLightGray}0.008 & \color{lsLightGray}0.005 & \color{lsLightGray}1007.838 & \color{lsLightGray}1.700 & \color{lsLightGray}0.089 \\ 
...
\begin{table}
	...
	\label{model ly complex experiment}
		...
		\textsc{Environment}-\texttt{l\#l-$<$lel$>$ }&\color{lsLightGray} 0.013 & \color{lsLightGray}0.012 &\color{lsLightGray} 59.281 & \color{lsLightGray}1.047 &\color{lsLightGray} 0.300 \\ 
		
		\textsc{Environment}-\texttt{l\#l-$<$ll$>$ }& -0.020 & 0.007 & 71.391 & -2.929 & 0.005 \\ 
		
		\textsc{Environment}-\texttt{syll.l\#l-$<$ll$>$  }& \color{lsLightGray}-0.006 & \color{lsLightGray}0.007 & \color{lsLightGray}100.898 &\color{lsLightGray} -0.861 &\color{lsLightGray} 0.391 \\ 
		
		log\textsc{RelFreq }&\color{lsLightGray}$<$0.001 &\color{lsLightGray} 0.001 &\color{lsLightGray} 50.887 & \color{lsLightGray}-0.319 &\color{lsLightGray} 0.751 \\ 
		...
		\textsc{Accentuation}-\texttt{unaccented} &\color{lsLightGray} 0.003 &\color{lsLightGray} 0.002 & \color{lsLightGray}1106.120 & \color{lsLightGray}1.134 & \color{lsLightGray}0.257 \\ 
...
\label{model ly complete experiment}
		...
		\textsc{Environment}-\texttt{syll.l\#l-$<$ll$>$ }& \color{lsLightGray} 0.006 & \color{lsLightGray} 0.004 & \color{lsLightGray} 1542.984 &\color{lsLightGray}  1.517 & \color{lsLightGray} 0.130 \\ 
		
		\textsc{Environment}-\texttt{l\#l-$<$lel$>$} & 0.011 & 0.005 & 871.672 & 2.178 & 0.030 \\ 
		
		\textsc{Environment}-\texttt{\#l-$<$l$>$} &\color{lsLightGray}  0.003 &\color{lsLightGray}  0.003 & \color{lsLightGray} 797.578 & \color{lsLightGray} 0.799 & \color{lsLightGray} 0.425 \\ 
		...
		\textsc{Environment}-\texttt{l-$<$ll$>$ }& \color{lsLightGray} 0.004 &\color{lsLightGray}  0.004 & \color{lsLightGray} 759.135 & \color{lsLightGray} 0.840 & \color{lsLightGray} 0.401 \\ 
		
		\textsc{Accentuation}-\texttt{unaccented }& \color{lsLightGray} 0.001 & \color{lsLightGray} 0.001 & \color{lsLightGray} 1505.165 & \color{lsLightGray} 1.031 & \color{lsLightGray} 0.303 \\ 
		
		\textsc{PostPause}-\texttt{pause} & \color{lsLightGray} 0.002 &\color{lsLightGray}  0.002 & \color{lsLightGray} 1503.609 &\color{lsLightGray}  0.834 &\color{lsLightGray}  0.405 \\ 
	...
	\label{tbl: Overview of environment in the experimental study}
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular} {lrrrr}
				...
				\midrule			
				\isi{un-}\textsuperscript{1}  & 148~ms & 95~ms   & 51~ms&\\
				\isi{un-}\textsuperscript{2}  & 133~ms & 94~ms   & 54~ms&\\  
				\isi{in-}\textsuperscript{3}  & 86~ms& 87~ms & 53~ms &  \\ 
				\isi{in-}\textsuperscript{4}  & 57~ms& 98~ms & 46~ms  & \\ 
				\isi{im-}\textsuperscript{3}  & 98~ms& 86~ms & NA & \\ 
				\isi{im-}\textsuperscript{4}  & 74~ms& 84~ms & NA & \\ 			
				\isi{dis-}\textsuperscript{1}    & 133~ms& NA& 108 ~ms (103~ms)&\\
				\isi{dis-}\textsuperscript{2}    & 125~ms& NA& 105~ms (105~ms)&\\
				...
				%	&Double -Singleton & Double -Singleton &\\
				\midrule			
				\isi{un-}\textsuperscript{1}& 53~ms & 97~ms   &  1: 1.6&1: 3.0\\ 
				\isi{un-}\textsuperscript{2}& 39~ms & 80~ms   & 1: 1.4&1: 2.5\\ 
				
				\isi{in-}\textsuperscript{3}&-1~ms &33~ms&1: 1.0&1: 1.6\\ 
				\isi{in-}\textsuperscript{4}&-41~ms &11~ms&  1: 0.6& 1: 1.2\\ 
				
				\isi{im-}\textsuperscript{3}&12~ms &NA  &  1: 1.1&NA \\ 
				\isi{im-}\textsuperscript{4}&-10~ms &NA  & 1: 0.9 & NA\\ 
				
				\isi{dis-} \textsuperscript{1}&NA & 25~ms  (30~ms)&NA &1: 1.2 (1: 1.3)   \\
				\isi{dis-} \textsuperscript{2}&NA & 20~ms  (20~ms)& NA& 1: 1.2 (1: 1.2) \\
				
				%\isi{-ly}&- & - & &  \\			
				
				\midrule
				\\
				\multicolumn{5}{l}{ \footnotesize{Note that for \prefix{dis}, the predicted durations and the predicted singleton-\isi{geminate} ratios for the singleton fricative in}}\\
				...
				\multicolumn{5}{l}{ \footnotesize{with a \isi{stressed} base-initial syllable.}}\\
				\\				
				\multicolumn{5}{l}{\textsuperscript{1} \footnotesize{in \isi{accented} position}}\\
				\multicolumn{5}{l}{\textsuperscript{2} \footnotesize{in unaccented position}}\\
				\multicolumn{5}{l}{\textsuperscript{3} \footnotesize{with \isi{stressed} base-initial syllable}}\\
